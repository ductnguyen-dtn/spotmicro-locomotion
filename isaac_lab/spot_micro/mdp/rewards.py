...
def trot_symmetry_reward(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg, std: float) -> torch.Tensor:
    """Reward diagonal trot symmetry using joint velocities, without contact sensors.

    Expected joint order from asset_cfg:
    [front_left_leg, front_right_leg, rear_left_leg, rear_right_leg]

    The reward encourages:
    - front_left_leg to move like rear_right_leg
    - front_right_leg to move like rear_left_leg
    """
    asset: Articulation = env.scene[asset_cfg.name]
    joint_vel = wp.to_torch(asset.data.joint_vel)[:, asset_cfg.joint_ids]
    ...
    diagonal_sync_error = torch.square(fl - rr) + torch.square(fr - rl)
    return torch.exp(-diagonal_sync_error / std)
